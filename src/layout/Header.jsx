...
// Imported by file path, not the feature barrel: the barrel also exports
// QrScannerKiosk, which would pull the QR scanner library into Header's
// module graph for a button that only opens a chooser.
import KioskSelectModal from '../features/kiosk/KioskSelectModal';
...
const Header = ({ title, subtitle }) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-[41] bg-dark-800/80 backdrop-blur-md border-b border-dark-700">
        <div className="flex min-w-0 items-center gap-3 px-4 py-4 sm:px-6">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {/* Kiosk — tablet (md) and up, hidden on mobile. Opens a chooser
                rather than jumping straight to the scanner, now that there are two
                kiosk modes. */}
            <button
            ...
            >
              ...
              <span className="hidden xl:inline text-sm font-medium">Kiosk</span>
            </button>
      ...
      <KioskSelectModal isOpen={showKioskModal} onClose={() => setShowKioskModal(false)} />
    </>
  );
};
