...
export const transformSessionsForCalendar = (sessions, handlers) => {
  ...
  return sessions.map((session) => {
    ...
    if (isCoachGroupClass) {
      ...
      title = session.className || 'Group Class';
      if (session.coach) {
      ...
      subtitle = session.className || 'Group Class';
      if (session.coach) {
        const coachName = session.coach.fullname || `${session.coach.firstname || ''} ${session.coach.lastname || ''}`.trim();
        meta.push({
          icon: User,
      ...
      // Coach PT: Show customer name as title, coach as subtitle
      if (session.customer) {
        title = `${session.customer.firstName || ''} ${session.customer.lastName || ''}`.trim() || 'PT Session';
      } else {
        title = 'PT Session';
      }
      if (session.coach) {
    ...
    } else if (isCoachPT || isMemberPT) {
      actions.onEdit = () => onSessionClick?.(session);
