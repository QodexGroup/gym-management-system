...
export const customerBillService = {
   ...
   */
  async create(billData, idempotencyKey = null) {
    const options = idempotencyKey ? { idempotencyKey } : {};
    const response = await postWithIdempotency(`${API_BASE_URL}/customers/bills`, billData, options);
   ...
   * @returns {Promise<Object>}
   */
  async update(id, billData, idempotencyKey = null) {
    const options = idempotencyKey ? { idempotencyKey } : {};
    const response = await putWithIdempotency(`${API_BASE_URL}/customers/bills/${id}`, billData, options);
