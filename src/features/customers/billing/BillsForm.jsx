...
const BillsForm = ({ customerId, currentMembership, onSubmit, onCancel, onCustomerUpdate, initialData = null, isSubmitting = false }) => {
  ...
  const isBillLocked = useMemo(() => {
    if (!isEditMode || !initialData || formData.billType !== BILL_TYPE.MEMBERSHIP_SUBSCRIPTION) return false;
    if (!currentMembership?.membershipStartDate) return false;
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex gap-3 pt-4">
        ...
        >
          {isSubmitting ? 'Saving...' : isEditMode ? 'Update Bill' : 'Generate Bill'}
        </button>
      </div>
    </form>
  );
