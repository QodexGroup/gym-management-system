...
const BillsTab = ({ member, onCustomerUpdate }) => {
  ...
  const { data: customerPtPackagesData } = useCustomerPtPackages(member?.id, {
  ...
  const handleAssignPtPackage = useCallback(async (packageData) => {
    try {
      await assignPtPackageMutation.mutateAsync({ customerId: member.id, packageData });
  ...
  const handleCancelPtPackage = useCallback(async (packageId) => {
    ...
    if (!result.isConfirmed) return;
    try {
      await cancelPtPackageMutation.mutateAsync({ customerId: member.id, packageId });
      queryClient.invalidateQueries({ queryKey: customerBillKeys.byCustomer(member.id) });
      onCustomerUpdate?.();
    } catch (error) {
      if (import.meta.env.DEV) console.error(error);
      ...
      <div className="card">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() => setShowPtPackageModal(true)}
            ...
            {hasActiveMembership ? (
              <div className="space-y-1.5 text-xs text-dark-300">
                ...
                {currentMembership.membershipPlan && (
                  <div title="Billing period from the membership plan">
