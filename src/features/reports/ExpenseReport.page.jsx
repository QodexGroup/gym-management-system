...
const ExpenseReportPage = () => {
  ...
  const handleExportPdf = async () => {
    try {
      const res = await reportService.checkExportSize({ reportType: 'expense', dateFrom: appliedFrom, dateTo: appliedTo });
      if (res.tooLarge) {
        await Alert.warning('Data is too large', 'We will send the report to your email (PDF).', { confirmButtonText: 'OK' });
        await reportService.emailReport({ reportType: 'expense', dateRange: 'custom', dateFrom: appliedFrom, dateTo: appliedTo, format: 'pdf' });
  ...
  const handleExportExcel = async () => {
    try {
      ...
      if (res.tooLarge) {
        ...
        await reportService.emailReport({ reportType: 'expense', dateRange: 'custom', dateFrom: appliedFrom, dateTo: appliedTo, format: 'excel' });
        Toast.success('Report request submitted. You will receive it by email.');
      } else {
        doExportExcel();
      }
              ...
              {monthlyTrend.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={monthlyTrend}>
                    ...
                    <Tooltip formatter={(value) => formatCurrency(value)} contentStyle={CHART_TOOLTIP_STYLE} />
                    <Bar dataKey="amount" fill="#0ea5e9" radius={[4, 4, 0, 0]} cursor={CHART_CURSOR} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <div className="flex items-center justify-center h-full text-dark-400">No data</div>
